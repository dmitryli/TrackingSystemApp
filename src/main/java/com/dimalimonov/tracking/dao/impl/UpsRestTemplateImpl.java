...
public class UpsRestTemplateImpl extends RestTemplate implements CarrierRestTemplate {
	...
	private final String TRACKING_REQ_TEMPLATE = "<?xml version=\"1.0\"?>" + "<AccessRequest>"
			+ "<AccessLicenseNumber>%s</AccessLicenseNumber>" + "<UserId>%s</UserId>" + "<Password>%s</Password>" + "</AccessRequest>"
			+ "<?xml version=\"1.0\"?>" + "<TrackRequest>" + "<Request>" + "<TransactionReference>"
			+ "<CustomerContext>simple test dima</CustomerContext>" + "<XpciVersion>1.0</XpciVersion>" + "</TransactionReference>"
